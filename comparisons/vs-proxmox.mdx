...
| **IaC** | Native (OpenTofu) | API/Terraform provider |
...
<Tabs>
  ...
  <Tab title="Proxmox + kombify">
    ...
    4. Use Easy Wizard or write `kombination.yaml`
    5. Test with kombify Sim
    6. Deploy ✅
...
**Using both together:** Proxmox handles node-level failures (hardware issues), while kombify's ha-homelab StackKit handles service-level resilience.
...
<Steps>
  ...
  <Step title="Create VMs for kombify">
    ...
    - 1 VM for kombify Stack controller
    - 1+ VMs for worker nodes
  </Step>
  <Step title="Install kombify Stack">
...
<CardGroup cols={2}>
  ...
  <Card title="Already have Proxmox?" icon="server" href="/stack/install/docker">
