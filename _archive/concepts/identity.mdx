...
graph TB
    ...
    subgraph Broker["🔗 Identity Broker"]
        tinyauth["tinyauth<br/>(OIDC Federation)"]
    end
    
    subgraph Local["🏠 Homelab Layer"]
...
kombify defines roles at two levels: SaaS (kombify Cloud) and homelab (kombify Stack).

### SaaS roles (kombify Cloud)
...
Authentication via **mTLS** (SPIFFE IDs) or OAuth2 client credentials against **kombify Gateway**.
...
- Combined flow: Cloudflare Tunnel → reverse proxy → mTLS validation → OIDC login
...
|------|-------------|----------------|
| `passkeys_only` | Passkeys (WebAuthn) only | ✅ **Default** |
