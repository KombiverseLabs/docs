...
<Warning>
**Describes the pre-2026-06-08 architecture; a clean rebuild is in progress.** The
product surfaces and component names below (Brain router, Companions, Managers, the
`chat.kombify.io` web chat and `ai.kombify.io` portal) reflect the legacy system. The
current target is: agents implemented in **kombify-Agents** on Mastra + the Cloudflare
Agents SDK (ADK/Gemini as documented exceptions), the web product in **kombify-AI-Workbench**,
the native companion in **kombify-AI-Mobile**, and the AI server in **kombify-AI-Platform**
behind `api.kombify.io/v1/ai/*`. This page is being rewritten to match.
</Warning>
