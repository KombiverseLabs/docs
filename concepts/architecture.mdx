...
graph TB
    ...
    subgraph Sphere["kombify Sphere (Optional SaaS)"]
        direction LR
        Auth["Zitadel SSO"]
    ...
    style Sim fill:#d1fae5,stroke:#059669
    style StackKits fill:#fce7f3,stroke:#db2777
...
graph LR
    ...
    style Sim fill:#d1fae5,stroke:#059669
    style Containers fill:#f3f4f6,stroke:#6b7280
```
...
graph TB
    ...
    subgraph Sphere["kombify Sphere"]
        direction TB
...
### Authentication Flow (Sphere)

```mermaid
sequenceDiagram
    participant User
    participant Sphere as kombify Sphere
    participant Zitadel
    participant Stack as kombify Stack
    ...
    Zitadel-->>Sphere: ID Token + Access Token
    
    User->>Sphere: Launch Stack
    Sphere->>Stack: Forward request + Token
...
<Columns cols={2}>
  ...
  <Card title="Hybrid (Stack + Sphere)" icon="cloud">
    Use kombify Sphere for SSO and billing while self-hosting Stack.
    
    **Best for:** Teams, enterprise users
    
    Connect via Sphere dashboard
  </Card>
</Columns>
